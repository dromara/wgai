...
public class MappingGridService {
     ...
     *
     * ⚠ 默认 1 = **不过滤**，这是现场定的。原因：帧数门限分不清"走过的人"和
     *   "只被瞥了两眼的稀疏静态物"——细立柱、远处料堆边缘、雷达掠射角打到的墙面，
     *   本来回波就只有几帧，一律被当动态物切掉，出图直接缺一块，比留下个把行人严重得多。
     *   动态物现在交给人工框选擦除(erasedRects)处理：擦除是看着图圈的，圈错还能撤销，
     *   而帧数过滤砍掉什么是静默的、事后无从追查。
     *   现场想恢复过滤随时把 mapping.grid.min-hits 调回 2~3，或在前端「动态物过滤」框里改，
     *   两条路都是运行时生效，不用改代码。
     */
    @Value("${mapping.grid.min-hits:1}")
    private int minHits;
